...
namespace SuperStock.Services
{
    public class StockService
    {
        ...
        public StockService(IConfiguration configuration, IMongoClient mongoClient)
        {
            ...
            {
                // Initialize empty list if MongoDB connection fails
                LocalEquityList = new List<Equities>();
            }
        }
        ...
        public async Task InitializeUser(UserAccountModel user,string gameType = "")
        {
            UserEquityHolding userStockData = new UserEquityHolding();
            userStockData.Email = user.Email;
            userStockData.Funds = "1000000";
            userStockData.SymbolHoldings = "";
            userStockData.WatchList = "";
            userStockData.T1Holdings = "";
            userStockData.UserId = user.UserId;
            mongoUserHoldingCollection = mongoDatabase.GetCollection<UserEquityHolding>("UserStocksData" + gameType);
            await mongoUserHoldingCollection.InsertOneAsync(userStockData);                        
        }

        internal int GetRank(string email, List<UserEquityHolding> allUsers, string gameType="")
        {
            var assetDict = GetAllUsersCurrentAssets(allUsers,gameType);
            var rankDict = new Dictionary<string, int>();
            var rank = 1;
            foreach(var kvp in assetDict.OrderByDescending(x => x.Value))
            {
                rankDict.Add(kvp.Key.Email,rank++);
            }
        ...
        internal Dictionary<string, int> GetRankOfAllUsers(string email, List<UserEquityHolding> allUsers,string gameType = "")
        {
            var assetDict = GetAllUsersCurrentAssets(allUsers,gameType);
            var rankDict = new Dictionary<string, int>();
            var rank = 1;
            foreach (var kvp in assetDict.OrderByDescending(x => x.Value))
            {
                //var name = string.IsNullOrWhiteSpace(kvp.Key.UserName) || rankDict.ContainsKey(kvp.Key.UserName)? kvp.Key.Email : kvp.Key.UserName;                
                var name = kvp.Key.Email;
                rankDict.Add(name, rank++);
            }
        ...
        internal string GetFundsFromEmail(string email,string gameType="")
        {
            UserEquityHolding userStockData = new UserEquityHolding();
            mongoUserHoldingCollection = mongoDatabase.GetCollection<UserEquityHolding>("UserStocksData"+ gameType);
        ...
        internal List<CompetitionRankData> GetCompetitionData()
        {
            var competitionDataList = new List<CompetitionRankData>();
            var collection = mongoDatabase.GetCollection<CompetitionRankData>("CompetitionRank");
            return collection.Aggregate<CompetitionRankData>().ToList();
        }
        ...
        private string AddToWatchList(string watchList, string stock)
        {
            ...
            // Validate stock symbol format (alphanumeric, dots, hyphens, up to 20 chars)
        ...
        public void UpdateUserWatchList(string email, string stock, int action, string gameType="")
        {
            // Input validation
        ...
        public async Task<List<StockPriceModel>> GetUserWatchList(string email, string gameType = "")
        {
            UserEquityHolding userStockData = new UserEquityHolding();
            mongoUserHoldingCollection = mongoDatabase.GetCollection<UserEquityHolding>("UserStocksData"+gameType);
            userStockData = mongoUserHoldingCollection.Find(x => x.Email == email).FirstOrDefault();
        ...
        private Dictionary<UserEquityHolding, double> GetAllUsersCurrentAssets(List<UserEquityHolding> emailList,string gameType)
        {
            int rank = 1;
            int total = emailList.Count;
            var AssetDict = new Dictionary<UserEquityHolding, double>();
            foreach(var userModel in emailList)
            {
                AssetDict.Add(userModel, GetTotalAssetValue(userModel.Email,gameType));
            }
        ...
        public double GetTotalAssetValue(string email,string gameType)
        {
            var availableFunds = Convert.ToDouble(GetFundsFromEmail(email,gameType));
            var EquityHoldings = GetUserEquityByEmail(email,gameType).GetAwaiter().GetResult();
        ...
        public async Task<UserEquityHolding> GetUserEquityByEmail(string email, string gameType="")
        {
            UserEquityHolding userStockData = new UserEquityHolding();
            mongoUserHoldingCollection = mongoDatabase.GetCollection<UserEquityHolding>("UserStocksData"+gameType);
            userStockData = mongoUserHoldingCollection.Find(x => x.Email == email).FirstOrDefault();
        ...
        private bool TryAdjustFunds(string email,OrderTypeVO Otype, double cost, string gameType = "")
        {
            var availableFunds = Convert.ToDouble(GetFundsFromEmail(email, gameType));
            mongoUserHoldingCollection = mongoDatabase.GetCollection<UserEquityHolding>("UserStocksData" + gameType);
            ...
            else
            {
                ...
                {
                    AddSubtractFunds(email, Math.Abs(cost),gameType);
                    return true;
                }
            }
            return false;
        ...
        private bool AddSubtractFunds(string email,double amount, string gameType="") {
            var previousFund = Convert.ToDouble(GetFundsFromEmail(email, gameType));
        ...
        public string UpdateUserEquityByEmail(string email, string stock, double price, int quantity, string gameType = "")
        {
            bool isBuy = quantity > 0;
            bool sufficientFunds;
            if (isBuy)
            {
                sufficientFunds = TryAdjustFunds(email, OrderTypeVO.Buy, price * quantity,gameType);
            }
            ...
            if (stockDict.ContainsKey(stock))
            {
                ...
                if (!isBuy )
                {
                    ...
                    {
                        TryAdjustFunds(email, OrderTypeVO.Sell, quantity * price, gameType);
                    }
                    else
                    {
